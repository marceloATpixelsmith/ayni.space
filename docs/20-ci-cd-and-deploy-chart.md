...
| `.github/workflows/codex-pr-hygiene.yml` | Enforce fresh-branch PR origin from latest `master` | Runs when in-repo PRs are opened/reopened against `master` | No deploy step |
| `.github/workflows/codex-safe-auto-merge.yml` | Enable native GitHub auto-merge for in-repo PRs after checks pass | Pull request target events on `master`-targeting PR updates | Merge automation only; no environment deploy |
