...
- `.github/workflows/pr-checks-summary.yml` is a separate top-level PR/push check (`PR Checks Summary Aggregator`) that uses a deterministic allowlist of top-level checks (`backend-gates`, `auth-security-regression-suite`, admin `test`, lockfile check, linear history check), excludes the summary check itself and non-allowlisted checks, scopes discovery to the exact current commit SHA, de-duplicates duplicate check-run names to the latest run, prints tracked check names up front, and during polling prints the exact unfinished tracked check names with status/conclusion before emitting one consolidated copy-pasteable fenced text block with deterministic `OVERALL RESULT` (`PASS`, `FAIL`, or `INCOMPLETE`) and explicit `FAILED CHECKS`, `UNFINISHED CHECKS`, and `PASSED CHECKS` sections.
