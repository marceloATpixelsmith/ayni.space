...
- `.github/workflows/pr-checks-summary.yml` is a separate top-level PR/push check (`PR Checks Summary Aggregator`) that uses a deterministic allowlist of top-level checks (`backend-gates`, `auth-security-regression-suite`, admin `test`, lockfile check, linear history check), excludes the summary check itself and non-allowlisted checks, scopes discovery to the exact current commit SHA, de-duplicates duplicate check-run names to the latest run, seeds the tracked list from the allowlist to prevent early-discovery omissions, and during polling merges newly discovered allowlisted checks before emitting one consolidated copy-pasteable fenced text block with deterministic `OVERALL RESULT` (`PASS`, `FAIL`, or `INCOMPLETE`) and explicit `FAILED CHECKS`, `UNFINISHED CHECKS`, and `PASSED CHECKS` sections.
- `.github/workflows/pr-checks-summary.yml` checks out repository contents (`actions/checkout@v4`) before running local Node helper scripts and includes a dedicated path-validation step that resolves + verifies `scripts/ci/pr-checks-summary-targeting-cli.mjs` and `scripts/ci/pr-checks-summary-targeting.mjs` so missing-path regressions fail clearly.
- `.github/workflows/ci-summary.yml` remains a backend-only summary workflow triggered by completion of `Backend Regression Gates`; it downloads `backend-gates-logs` from the exact upstream run (`github.event.workflow_run.id`) and emits one combined fenced text block to both logs and `$GITHUB_STEP_SUMMARY`.
