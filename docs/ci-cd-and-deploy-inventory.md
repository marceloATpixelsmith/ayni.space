...
- `.github/workflows/backend-ci-summary.yml` uses GitHub API check/status queries against both PR SHAs (`pull_request.head.sha` and `pull_request.merge_commit_sha`) and summarizes `backend-typecheck`, `backend-build-api`, `backend-api-tests`, `api-regression-suite`, and `auth-security-regression-suite` from the SHA where required checks are observed.
