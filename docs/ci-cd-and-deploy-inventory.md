...
- `.github/workflows/pr-checks-summary.yml` is a separate top-level PR/push check (`PR Checks Summary Aggregator`) that waits for targeted validation checks (`backend-gates`, `auth-security-regression-suite`, `api-regression`, admin `test`, lockfile check, and linear history check), then emits one consolidated copy-pasteable fenced text block with deterministic `OVERALL RESULT` (`PASS`, `FAIL`, or `INCOMPLETE`) and explicit `FAILED CHECKS`, `UNFINISHED CHECKS`, and `PASSED CHECKS` sections.
