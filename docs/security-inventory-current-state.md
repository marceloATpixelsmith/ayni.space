...
2. **Origin runtime contract mismatch resolved**: canonical app-origin config is now `platform.apps.domain`; `ALLOWED_ORIGINS` remains env-only extension input and no longer belongs in app settings.
