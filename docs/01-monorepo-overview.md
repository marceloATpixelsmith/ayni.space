...
- `.github/workflows/admin-security-shell-test-and-deploy.yml`:
  ...
  - Pull request path filters target `apps/admin/**` and lock/workspace metadata; push-to-`master` runs are unfiltered so deploy webhook can trigger on normal merges.
