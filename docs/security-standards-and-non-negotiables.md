...
- Shared DB client configuration now enforces CA/certificate validation in production by default (`ssl.rejectUnauthorized=true`), but honors explicit `DATABASE_URL` TLS intent (`sslmode=require|no-verify`) for managed providers that only expose self-signed/private certificate chains; non-production behavior remains explicit (`ssl=false`) in `lib/db/src/index.ts`.
