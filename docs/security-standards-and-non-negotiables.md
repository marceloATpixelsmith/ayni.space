...
- Shared DB client configuration now enforces CA/certificate validation in production by default (`ssl.rejectUnauthorized=true`), downgrades to encrypted/no-verify when `DATABASE_URL` explicitly requests `sslmode=require|no-verify`, and also applies the same no-verify fallback on Render-managed runtime unless `sslmode=verify-ca|verify-full` is explicitly set; non-production behavior remains explicit (`ssl=false`) in `lib/db/src/index.ts`.
