...
- `/api/auth/me` now supports a safe MFA-pending bootstrap contract (HTTP 200 plus `mfaPending`, `mfaEnrolled`, and `nextStep`) by allowing pending-session identity resolution on `/me` while preserving MFA gating on protected endpoints; frontend MFA enrollment bootstrap now honors `nextStep` and redirects to challenge when the pending session is already enrolled (`apps/api-server/src/middlewares/requireAuth.ts`, `apps/api-server/src/routes/auth.ts`, `apps/admin/src/pages/auth/MfaEnroll.tsx`).
