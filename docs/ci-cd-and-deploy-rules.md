...
- PR validation workflows:
  ...
  - Backend gate failures are consolidated into one fenced `$GITHUB_STEP_SUMMARY` block (tailing each log) so backend/auth failure details are copyable from one place without artifact downloads.
