...
- PR validation workflows:
  ...
  - Backend gate failures are rendered by a separate `ci-summary` job that runs after `backend-gates`, downloads `backend-gates-logs`, and emits one combined fenced text block (tailing each log) to both logs and `$GITHUB_STEP_SUMMARY`.
