...
- PR validation workflows:
  ...
  - `.github/workflows/backend-regression-gates.yml` runs backend typecheck/build/test gates on PRs affecting backend/shared package scope, captures each backend gate command's raw output into `artifacts/backend-gates/*.log` via `set +e` + `set -o pipefail` + `tee`, records each real command status to `$GITHUB_OUTPUT` as `exit_code`, exits each gate step with that same code, uploads raw logs as artifacts, and on any failed gate runs `scripts/ci/summarize-backend-test-failures.mjs` to print step-labeled failure content directly in action output and append the same content to `$GITHUB_STEP_SUMMARY`.
  - Backend failure summarization now includes a robust fallback: if structured parsing cannot extract useful failure data, the summary prints `structured parse: failed` and emits a raw failsafe excerpt selected from either marker-based failure blocks (`not ok`, `FAIL`, `Error:`, `AssertionError`, `ERR_*`, exit-code markers) or the last meaningful log tail.
