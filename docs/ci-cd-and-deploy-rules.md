...
- PR validation workflows:
  ...
  - `.github/workflows/pr-checks-summary.yml` adds a separate top-level check (`PR Checks Summary Aggregator`) for PR/push events; it waits until targeted validation checks complete, then prints one consolidated fenced text block in logs + `$GITHUB_STEP_SUMMARY` with deterministic overall state (`OVERALL RESULT: PASS|FAIL|INCOMPLETE`), plus separate `FAILED CHECKS`, `UNFINISHED CHECKS`, and `PASSED CHECKS` buckets.
