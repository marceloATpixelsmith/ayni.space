...
- PR validation workflows:
  ...
  - `.github/workflows/pr-checks-summary.yml` adds a separate top-level check (`PR Checks Summary Aggregator`) for PR/push events; it waits until targeted validation checks complete, then prints one consolidated fenced text block in logs + `$GITHUB_STEP_SUMMARY` with pass/fail status and harvested failure snippets for each failed check.
