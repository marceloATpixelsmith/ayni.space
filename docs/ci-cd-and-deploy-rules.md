...
- PR validation workflows:
  ...
  - `.github/workflows/ci-summary.yml` is a dedicated top-level check that triggers on `workflow_run` completion for `Backend Regression Gates`, downloads `backend-gates-logs` from that exact run ID, and emits one combined fenced text block (tailing each log) to both logs and `$GITHUB_STEP_SUMMARY`.
