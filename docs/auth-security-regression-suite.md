...
- Missing all context candidates fails closed with `app_slug_missing`; canonical slug lookup returning null fails with `app_not_found` outside the explicit test-mode known-app fallback; canonical lookup exceptions fail as `app_context_unavailable` for request/origin sources and `app_not_found` for session-group fallback source (`apps/api-server/src/lib/authContextPolicy.ts`, `apps/api-server/src/__tests__/auth-app-context-policy.test.ts`).
...
- Test-mode canonical app fallback is centralized and allowlist-based for auth-entry coverage: `admin`, `workspace`, `workspace-solo`, `org-open`, `solo-app`, `ayni`, and `shipibo` resolve to complete app objects with fixed access mode/session group metadata, while unknown slugs remain fail-closed. OAuth start additionally allows the route-level known fallback only for explicit `admin`/`workspace` app slugs or default/workspace origin-derived context; admin origin context without explicit `appSlug=admin` remains fail-closed (`apps/api-server/src/lib/appAccess.ts`, `apps/api-server/src/routes/auth.ts`).
...
- OAuth-start auth-entry tests that are not validating Turnstile use shared request-scoped test setup to mark the request as already Turnstile-verified, so ambient `TURNSTILE_ENABLED`/production-safe defaults cannot turn unrelated app-context or session-group assertions into false `403` failures; dedicated Turnstile tests keep validating missing/invalid-token fail-closed behavior.
...
- Auth i18n drift is covered by frontend tests: `apps/admin/src/__tests__/auth-i18n.runtime.test.tsx` verifies default English rendering, `formatAuthMessage`/`useAuthI18n().format` interpolation, shared validation/runtime messages through typed auth translation keys, missing-key detection for admin auth pages/shared auth runtime, and fallback-heavy/hardcoded visible-copy guards. `apps/admin/src/__tests__/security-shell.contract.test.mjs` locks key-based admin auth source contracts without changing visible English copy.
- Auth i18n remains English-only for now. Translation keys live in `lib/auth-ui/src/locales/en/auth.ts`; `AuthI18nProvider` and `useAuthI18n` are the current auth-localization foundation; `lib/frontend-security` may consume the same auth messages for shared validation/runtime messages. This pass does not add language switching, Spanish translations, backend localization, or auth route response-shape changes.
