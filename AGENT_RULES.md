...
- Do not reintroduce force-reset/force-push promotion behavior unless explicitly requested by the repository owner.
- Suggest a different governance model only if the user explicitly asks for it.

### Do not break
- Do not claim force-reset or destructive overwrite of `master` is acceptable governance in this repository unless the owner explicitly requests it.
