...
function decideMfaScreenModeFromInputs(params: {
  ...
  if (
    ...
    (params.authMfaPending &&
      params.authMfaEnrolled &&
      params.authNextStep !== "mfa_enroll")
  ) {
  ...
  if (!params.meResponseOk) {
    return {
      mode: "challenge",
      reason: "auth_me_request_failed_challenge_fail_closed",
    };
  }
  ...
  if (!params.mePayloadReadable) {
    return {
      mode: "challenge",
      reason: "auth_me_payload_missing_challenge_fail_closed",
    };
  }

  if (
    params.mePayload?.mfaPending &&
    params.mePayload.nextStep === "mfa_challenge"
  ) {
...
function MfaEnrollContent() {
  ...
  const enrollmentStartRef = React.useRef<Promise<
    Awaited<ReturnType<typeof auth.startMfaEnrollment>>
  > | null>(null);
  const [phase, setPhase] = React.useState<
    "initializing" | "ready" | "submitting" | "success" | "init-error"
  >("initializing");
  ...
  const authSnapshotRef = React.useRef({
    ...
    nextStep:
      auth.user?.nextStep ?? (null as "mfa_enroll" | "mfa_challenge" | null),
  });
    ...
    void secureApiFetch("/api/auth/me", {
      ...
      .then(async (response) => {
        ...
        if (response.status === 401) {
          setPhase("init-error");
          setInitError(t("mfa_enroll_session_inactive"));
          return;
        }
          ...
          .catch((err) => {
            ...
            setInitError(
              err instanceof Error
                ? err.message
                : t("mfa_enroll_init_error_fallback"),
            );
          })
          .finally(() => {
  ...
  return (
    <AuthShell
      title={t("mfa_enroll_title")}
      subtitle={t("mfa_enroll_subtitle")}
            ...
            <>
              <ol className="text-sm text-muted-foreground space-y-2 list-decimal pl-5">
                <li className="flex items-start gap-2">
                  ...
                  {t("mfa_enroll_step_download_app")}
                </li>
                <li>
                  {t("mfa_enroll_step_scan_qr")}
                </li>
                <li>
                  {t("mfa_enroll_step_enter_first_code_prefix")}{" "}
                  <strong>
                    {t("mfa_enroll_step_enter_first_code_emphasis")}
                  </strong>{" "}
                  {t("mfa_enroll_step_enter_first_code_suffix")}
                </li>
              </ol>
              <p className="text-sm text-muted-foreground">
                {format("mfa_enroll_account_issuer", { issuer })}
              </p>
              {qrCodeUrl ? (
              ...
              <p className="text-xs text-muted-foreground">
                {t("mfa_enroll_manual_setup")}{" "}
                <code className="break-all">{secret}</code>
              </p>
              <input
                autoFocus
                className="w-full border rounded px-3 py-2"
                placeholder={t("mfa_enroll_code_placeholder")}
                value={code}
                onChange={(e) => setCode(e.target.value)}
                aria-invalid={Boolean(submitError)}
                aria-describedby={
                  submitError ? "twostep-enroll-error" : undefined
                }
              />
              <FieldValidationMessage
                id="twostep-enroll-error"
                message={submitError}
              />
              <Button
                className="w-full"
              ...
              >
                {phase === "submitting"
                  ? t("mfa_enroll_verify_loading")
                  : t("mfa_enroll_verify_button")}
              </Button>
              {recovery.length > 0 ? (
                <div className="space-y-2">
                  <p className="text-sm font-medium">
                    {t("mfa_enroll_recovery_codes_label")}
                  </p>
