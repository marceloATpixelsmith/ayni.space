...
function MfaEnrollContent() {
      ...
      .catch((err) => {
        ...
        setSubmitError(
          err instanceof Error
            ? err.message
            : t(
                "mfa_enroll_verify_fallback",
                "Unable to verify two-step verification code.",
              ),
        );
      });
  };
  ...
  return (
    <AuthShell
      title={t("mfa_enroll_title", "Set up two-step verification")}
      subtitle={t(
        "mfa_enroll_subtitle",
        ...
        <div className="space-y-3">
          {phase === "initializing" ? (
            <p className="text-sm text-muted-foreground">
              {t("mfa_enroll_preparing", "Preparing your authenticator setup…")}
            </p>
          ) : null}
          <AuthStatusMessage
            message={
          ...
          {phase === "init-error" ? (
            <Button className="w-full" onClick={() => window.location.reload()}>
              {t("mfa_enroll_retry_setup", "Retry setup")}
            </Button>
          ) : null}
          ...
          {phase === "init-error" ? null : (
            <>
              <ol className="text-sm text-muted-foreground space-y-2 list-decimal pl-5">
                ...
                <li>
                  {t(
                    "mfa_enroll_step_scan_qr",
                    "Scan the QR code shown below.",
                  )}
                </li>
                <li>
                  {t("mfa_enroll_step_enter_first_code_prefix", "Enter the")}{" "}
                  <strong>
                    {t(
              ...
              <input
                ...
                placeholder={t("mfa_enroll_code_placeholder", "6-digit code")}
                value={code}
              ...
              >
                {phase === "submitting"
                  ? t("mfa_enroll_verify_loading", "Verifying…")
                  : t(
                      "mfa_enroll_verify_button",
                      "Verify and activate two-step verification",
                    )}
              </Button>
              {recovery.length > 0 ? (
                <div className="space-y-2">
                  <p className="text-sm font-medium">
                    {t(
                      "mfa_enroll_recovery_codes_label",
                  ...
                  >
                    {t("mfa_enroll_continue_button", "Continue")}
                  </Button>
                </div>
              ) : null}
            </>
