...
function validateHexKey(name: string, value: string) {
  if (!/^[a-fA-F0-9]{64}$/.test(value)) {
    throw new Error(`${name} must be exactly 64 hex characters.`);
  }
}

function validateLane1ProviderEnv() {
  const provider = process.env["PLATFORM_TRANSACTIONAL_EMAIL_PROVIDER"] ?? "brevo";
  if (provider !== "brevo") throw new Error(`PLATFORM_TRANSACTIONAL_EMAIL_PROVIDER must be "brevo" when set. Received: ${provider}`);

  const apiKey = process.env["PLATFORM_BREVO_API_KEY"];
  if ("PLATFORM_BREVO_API_KEY" in process.env && !apiKey) throw new Error("PLATFORM_BREVO_API_KEY is set but empty.");
...
export function validateEnv() {
  ...
  validateHexKey("EMAIL_CREDENTIALS_ENCRYPTION_KEY", process.env["EMAIL_CREDENTIALS_ENCRYPTION_KEY"] as string);
  validateHexKey("MFA_TOTP_ENCRYPTION_KEY", process.env["MFA_TOTP_ENCRYPTION_KEY"] as string);
