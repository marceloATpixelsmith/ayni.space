-- Canonicalize runtime settings with only currently valid DB-backed keys.
-- This pass intentionally does NOT seed deprecated app_settings keys
-- (ALLOWED_ORIGIN/ALLOWED_ORIGINS/VITE_TURNSTILE_SITE_KEY).
...
WITH app_seed AS (
  ...
  SELECT app_id, app_slug, 'MFA_ISSUER'::text AS key, mfa_issuer::text AS value, 'string'::text AS value_type, 'MFA issuer display label for app'::text AS description FROM app_rows
  UNION ALL SELECT app_id, app_slug, 'VITE_AUTH_DEBUG', vite_auth_debug, 'boolean', 'Frontend auth debug panel toggle' FROM app_rows
  UNION ALL SELECT app_id, app_slug, 'VITE_SENTRY_ENVIRONMENT', vite_sentry_environment, 'string', 'Frontend Sentry environment label' FROM app_rows
  UNION ALL SELECT app_id, app_slug, 'VITE_SENTRY_DSN', vite_sentry_dsn, 'string', 'Frontend Sentry DSN (non-secret runtime value)' FROM app_rows
  UNION ALL SELECT app_id, app_slug, 'BASE_PATH', base_path, 'string', 'Frontend app base path' FROM app_rows
  UNION ALL SELECT app_id, app_slug, 'VITE_API_BASE_URL', vite_api_base_url, 'string', 'Frontend API base URL bootstrap/runtime mirror' FROM app_rows
  UNION ALL SELECT app_id, app_slug, 'VITE_APP_SLUG', vite_app_slug, 'string', 'Frontend app slug bootstrap/runtime mirror' FROM app_rows
)
...
-- Remove legacy plural origin key if present. Domain canonicalization happens in
-- 20260421_canonical_app_config.sql.
