...
WITH seeded AS (
  SELECT * FROM (VALUES
    ...
    ('ayni', 'https://ayni.ayni.space', 'Ayni Workspace'),
    ('shipibo', 'https://shipibo.ayni.space', 'Ayni Shipibo'),
...
SELECT 'app_setting_allowed_origins_' || app_rows.slug, app_rows.id, 'ALLOWED_ORIGINS', app_rows.origin, 'string', 'Allowed browser origin for app'
FROM app_rows
