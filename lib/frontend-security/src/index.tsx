...
*/
  ...
  const acceptInvitationWithPassword = React.useCallback(
    async (token: string, password: string, turnstileToken?: string | null) => {
      ...
      if (!response.ok) {
        const error = new Error(
          payload?.error ??
            getAuthMessage("invitation_password_set_failed_sentence"),
        ) as Error & { code?: string; status?: number };
        error.code = payload?.code;
        error.status = response.status;
        throw error;
    ...
    async (
      ...
      if (!response.ok) {
        ...
        if (payload?.code === "POST_AUTH_DESTINATION_UNRESOLVED") {
          throw new Error(
            getAuthMessage("login_error_post_auth_destination_unresolved"),
          );
        }
        throw new Error(
  ...
  const forgotPassword = React.useCallback(
    async (email: string) => {
      ...
      if (!response.ok)
        throw new Error(
          payload?.error ?? getAuthMessage("auth_error_process_request"),
        );
  ...
  const completeMfaChallenge = React.useCallback(
    async (code: string, rememberDevice: boolean, stayLoggedIn = false) => {
      ...
      if (!response.ok)
        throw new Error(
          payload?.error ?? getAuthMessage("mfa_challenge_complete_error"),
        );
      const nextPath = sanitizePostAuthNavigationPath(payload?.nextPath);
      if (nextPath) {
        await finalizePostAuthNavigation(nextPath);
  ...
  const completeMfaRecovery = React.useCallback(
    async (
      ...
      if (!response.ok)
        throw new Error(
          payload?.error ?? getAuthMessage("mfa_recovery_complete_error"),
        );
      const nextPath = sanitizePostAuthNavigationPath(payload?.nextPath);
      if (nextPath) {
        await finalizePostAuthNavigation(nextPath);
